...
- [x] **SOL-36 "Zod alien" column bug** — `COL_ORANGE = 10` corrupts the BACKTAB GRAM card field when used in `GRAM_SOL36 * 8 + AlienColor + $0800`. In Color Stack mode, bits 3-8 encode the card number; adding 10 shifts card 46 → card 47 (GRAM_ORBITER = SmallCrabF1Gfx = the small crab). This caused a column of orange "Zod crabs" to flash at the beam column during the second kill spurt (T=14→5). Fixed by replacing `COL_ORANGE` with `COL_RED` (2) — color 2 appears orange-reddish on the Intellivision and keeps visual contrast with the yellow spurt-1. **Rule: only use colors 0-7 in direct BACKTAB GRAM card write formulas.** waves.bas Sol36SputterUpdate.
- [x] **SOL-36 stale beam column after player death** — `Sol36SputterStop` only zeroed the timer but never called `Sol36Clear`, leaving the beam column tiles on screen. Also, `Sol36BeamTimer` was not reset in the death handlers so the beam kept redrawing. Fixed: `Sol36Clear` added to `Sol36SputterStop`; `Sol36BeamTimer = 0` added to both death paths in gameloop.bas.
- [x] **SOL-36 fizzle phase shows stale tile flash** — The 4-frame fizzle (T=4→1) exposed underlying BACKTAB content at the beam column. Fixed by skipping the fizzle entirely: `IF Sol36SputterTimer < 5 THEN Sol36SputterTimer = 0` jumps straight to T=0 cleanup. waves.bas.
